 ...
 */
export function InlangPattern(props: {
  ...
  const sourceVariant = () =>
    getVariant(props.message, { where: { languageTag: props.refLanguage } })
  ...
  */
  ...
  //   on(lastPullTime, () => {
  //     setReferencePattern(
  //       props.message.variants.find((variant) => variant.languageTag === props.refLanguage)?.pattern
  //     )
  //     setHasChanges(false)
  //   })
  // )
  ...
  const autoSave = () => {
    ...
    if (newMessage.data) {
      const upsertSuccessful = props.handleSave(newMessage.data)
      if (!upsertSuccessful) {
  ...
  const handleMachineTranslate = async () => {
    ...
    const translation = await rpc.machineTranslateMessage({
      message: newMessage,
      sourceLanguageTag: props.refLanguage,
  ...
  const getNotificationHints = () => {
    ...
    props.lintReports.map((report) => {
      if (report.messageId === props.message.id && report.languageTag === props.languageTag) {
        ...
        const messageLintRuleName = report.ruleId
        notifications.push({
          notificationTitle:
    ...
    if (hasChanges() && !props.userLoggedIn) {
      notifications.push({
    ...
    if (hasChanges() && props.userLoggedIn && props.disableSave) {
      notifications.push({
    ...
    >
      <div class="flex justify-start items-start gap-2 py-[5px]">
        <div class="flex justify-start items-center flex-grow-0 flex-shrink-0 w-[72px] gap-2 py-0">
          ...
          {props.refLanguage === props.languageTag && (
        ...
        <div class="flex items-center justify-end gap-2">
          ...
          <Show when={hasChanges() && isLineItemFocused()}>
            <sl-button
              ...
              prop:size="small"
              prop:disabled={hasChanges() === false || props.disableSave}
              onClick={() => {
